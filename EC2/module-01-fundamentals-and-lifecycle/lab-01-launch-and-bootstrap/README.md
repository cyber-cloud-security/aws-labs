...
## 📌 Lab Objectives
- Understand the difference between x86_64 (`Intel/AMD`) and ARM64 (`AWS Graviton`) EC2 architectures.
- Launch an EC2 instance programmatically using the AWS CLI.
- Bootstrap the instance with automated **User Data** (`cloud-init`) to install Nginx and query instance metadata.
- Configure Security Groups with least-privilege ingress (HTTP port 80).
- Query instance metadata using **IMDSv2** (Instance Metadata Service Version 2).
