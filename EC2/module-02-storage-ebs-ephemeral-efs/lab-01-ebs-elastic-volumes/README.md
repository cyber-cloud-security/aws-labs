...
## 📌 Lab Objectives
- Create, attach, and persistently mount an Amazon EBS volume (`gp3`) to an EC2 instance.
- Safely configure `/etc/fstab` using filesystem **UUIDs** and fail-safe options (`nofail`) to prevent boot hangs.
- Perform **Online Elastic Volume Expansion**: Increase volume size from 5 GiB to 10 GiB on-the-fly without unmounting or rebooting.
- Expand partition table (`growpart`) and resize file systems (`xfs_growfs` / `resize2fs`) with zero downtime.
...
export AZ=$(aws ec2 describe-subnets --subnet-ids "${SUBNET_ID}" --query "Subnets[0].AvailabilityZone" --output text)
