...
## 📌 Lab Objectives
- Enable **VPC Flow Logs** targeting an EC2 instance's Elastic Network Interface (ENI).
- Deliver flow logs to an **Amazon CloudWatch Logs** log group.
- Generate intentional network drops (security group block) and inspect the resulting `REJECT` log records.
- Author **CloudWatch Logs Insights** queries to identify top talkers, malicious port scans, and firewall drop anomalies.
