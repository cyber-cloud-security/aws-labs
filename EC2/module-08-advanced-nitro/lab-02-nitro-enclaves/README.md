...
## 📌 Lab Objectives
- Understand the security architecture of **AWS Nitro Enclaves** for confidential computing.
- Launch an EC2 instance configured with `--enclave-options Enabled=true`.
- Install the **Nitro Enclaves CLI** (`nitro-cli`) and allocate dedicated CPU cores and RAM to the enclave subsystem.
- Build an **Enclave Image File (`.eif`)** from a minimal container and execute it.
- Understand how **Cryptographic Attestation** allows AWS KMS to conditionally release cryptographic secrets directly inside the enclave via local `vsock`.
