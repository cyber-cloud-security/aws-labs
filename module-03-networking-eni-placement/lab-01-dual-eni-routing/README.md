...
## 📌 Lab Objectives
- Provision and attach a secondary **Elastic Network Interface (ENI)** to an EC2 instance across two subnets.
- Understand how primary (`eth0`) and secondary (`eth1`) interfaces interact.
- Diagnose and resolve the classic Linux **asymmetric routing problem** (where return packets exit the wrong interface and get dropped).
- Implement Linux policy-based routing (`ip rule`, `ip route`) to ensure traffic responding to `eth1` exits through `eth1`'s gateway.
