...
## 📌 Lab Objectives
- Implement the **Zero-Credential Security Architecture**: completely eliminate static AWS access keys on EC2 instances.
- Construct an IAM Role with an `ec2.amazonaws.com` assume-role trust policy.
- Package the IAM Role into an **Instance Profile** and attach it to an active, running EC2 instance with zero downtime.
- Inspect how the EC2 instance automatically rotates and manages temporary **AWS STS credentials** via IMDSv2.
...
aws iam detach-role-policy --role-name "ec2-labs-s3-reader-role" --policy-arn "arn:aws:iam::aws:policy/AmazonS3ReadOnlyAccess"
