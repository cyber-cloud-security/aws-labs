...
## 📌 Lab Objectives
- Implement the modern **"Zero-SSH / Bastion-less" Enterprise Architecture**:
...
- Attach the `AmazonSSMManagedInstanceCore` managed policy to an EC2 instance profile.
- Establish encrypted interactive terminal sessions directly through the AWS CLI using `aws ssm start-session`.
- Configure **SSM Port Forwarding** to tunnel local workstation ports securely to private EC2 instances.
...
aws iam remove-role-from-instance-profile --instance-profile-name "ec2-ssm-core-profile" --role-name "ec2-ssm-core-role"
aws iam delete-instance-profile --instance-profile-name "ec2-ssm-core-profile"
aws iam detach-role-policy --role-name "ec2-ssm-core-role" --policy-arn "arn:aws:iam::aws:policy/AmazonSSMManagedInstanceCore"
