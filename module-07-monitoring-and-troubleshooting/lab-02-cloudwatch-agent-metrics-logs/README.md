...
## 📌 Lab Objectives
- Understand the **Hypervisor Visibility Boundary**: why AWS CloudWatch cannot view internal OS metrics (RAM, swap, disk filesystem usage) by default.
- Deploy the **Unified Amazon CloudWatch Agent** inside an EC2 instance.
- Configure metric collection for `mem_used_percent`, `swap_used_percent`, and `disk_used_percent`.
- Stream local Linux system logs (`/var/log/messages`) into a centralized **Amazon CloudWatch Log Group**.
